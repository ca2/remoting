...
namespace remoting_macos
{
   ...
   void MacosUserInput::getApplicationRegion(const ::process_identifier & processidentifier, ::remoting_rfb::Region & region)
   {
      region.clear();
      ::remoting_rfb::Region occludedRegion;
      auto windowList = CGWindowListCopyWindowInfo(kCGWindowListOptionOnScreenOnly, kCGNullWindowID);
      if (!windowList)
      {
         return;
      ...
      for (CFIndex i = 0; i < count; ++i)
      {
         ...
         {
            ::remoting_rfb::Region windowRegion(rectangle);
