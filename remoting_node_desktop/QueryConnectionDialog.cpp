...
void QueryConnectionDialog::updateTimeoutLabel()
{
  ...
  if (m_acceptByDefault) {
    labelText.format(MainSubsystem().StringTable().getString(IDS_AUTO_ACCEPT_CONNECTION_FORMAT), m_timeout);
  } else {
    labelText.format(MainSubsystem().StringTable().getString(IDS_AUTO_REJECT_CONNECTION_FORMAT), m_timeout);
  }
