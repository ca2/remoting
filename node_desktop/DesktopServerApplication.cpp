    ...
    m_plogwriter(::system())
{
  ...
  try {
    ...
    while (mem[0] == 0) {
      unsigned int timeForWait = maximum((int)10000 - 
                                     (int)(::earth::time::now() -
                                           startTime).m_iSecond,
                                     0);
      if (timeForWait == 0) {
    ...
    m_plogwriter->informationf("Client->server readPipeHandle = %p, writePipeHandle = %p", readPipeHandle, writePipeHandle);
    ...
    m_plogwriter->informationf("Server->client readPipeHandle = %p, writePipeHandle = %p", readPipeHandle, writePipeHandle);
    ...
    m_updHandlerSrv = new UpdateHandlerServer(m_srvToClGate, m_dispatcher, [this]() { onHappening(); }, m_plogwriter);
