...

{
   ...
   void DesktopBaseImpl::getDisplayNumberCoords(::int_rectangle *rect, unsigned char dispNumber)
   {
      _ASSERT(m_userInput != 0);
      _ASSERT(m_extDeskTermListener != 0);
   ...
   void DesktopBaseImpl::sendUpdate()
   {
      ...
      if (!updCont.is_empty() || !m_fullReqRegion.is_empty())
      {
         ...
         m_extUpdSendingListener->onSendUpdate(&updCont, m_updateHandler->getCursorShape());
