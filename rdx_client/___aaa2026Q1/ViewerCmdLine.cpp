...
#include "../framework.h"
#include "remoting/remoting_rtc/config/IniFileSettingsManager.h"
...
namespace remoting_client
{
    ...
    ViewerCmdLine::ViewerCmdLine(ConnectionData *pconnectiondata,
                                 ...
                                 ::remoting_rtc::ViewerConfig *config,
                                 bool *isListening)
    ...
    void ViewerCmdLine::parseOptionsFile()
    {
        ...
        ::remoting_rtc::IniFileSettingsManager sm(pathToIniFile);
    ...
    void ViewerCmdLine::parseEncoding()
    {
        if (isPresent(ENCODING)) {
            ::i32 iEncoding = ::remoting_rtc::EncodingDefs::RAW;

            if (m_options[ENCODING] == HEXTILE) {
                iEncoding = ::remoting_rtc::EncodingDefs::HEXTILE;
            }
            if (m_options[ENCODING] == TIGHT) {
                iEncoding = ::remoting_rtc::EncodingDefs::TIGHT;
            }
            if (m_options[ENCODING] == RRE) {
                iEncoding = ::remoting_rtc::EncodingDefs::RRE;
            }
            if (m_options[ENCODING] == ZRLE) {
                iEncoding =::remoting_rtc:: EncodingDefs::ZRLE;
            }
    ...
    void ViewerCmdLine::parseMouseShape()
    {
        if (isPresent(MOUSE_LOCAL)) {
            ::i32 localCursorShape = ::remoting_rtc::ConnectionConfig::DOT_CURSOR;
