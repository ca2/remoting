...
::innate_subsystem::ControlInterface * ViewerWindow::getControl()
    ...
    void ViewerWindow::changeCursor(::i32 type)
    {
        ...
        switch (type) {
           case ::remoting_rtc::ConnectionConfig::DOT_CURSOR:
    ...
    void ViewerWindow::commandSaveSession()
    {
       ...
       if (path.has_character())
       {
          try
          {
             ...
             ::remoting_rtc::IniFileSettingsManager sm(fileName);
...
system()->acme_windowing()->post([this]()
   ...
   system()->m_papplication->set_finish();
        ...
        if (wParam != ::remoting_client::AuthException::AUTH_CANCELED) {
            ...
            if (result == ::e_dialog_result_retry) {
                if (!m_pconnectiondata->isIncoming()) {
                    ...
                    auto pconnectionconfig = allocateø ::remoting_rtc::ConnectionConfig(*m_pconnectionconfig);
    ...
    bool ViewerWindow::onFsWarning()
    {
       ...
       pfullscreenwarningdialog->doAttachedModal([this](::i32 iResult)
                                 {
   ...
   bool ViewerWindow::onCalculateDefaultSize(::i32_rectangle & rectangle)
    ...
    void ViewerWindow::onConnected(::remoting_rtc::RfbOutputGate *output)
    {
        // Set flags.
        m_isConnected = true;
