...
namespace remoting_rdx_client
{
   ...
   //
   //
   // void rdp_host::control_activate(bool bActivate)
   // {
   //
   ...
   //
   //
   // void rdp_host::do_control_show_verb()
   // {
   //
   //    auto hwnd = ::as_HWND(this->operating_system_window());
   //
   ...
   //
   //
   // void rdp_host::do_control_ui_activate()
   // {
   //
   //    auto hwnd = ::as_HWND(this->operating_system_window());
   //
   ...
   //
   //
   // void rdp_host::on_window_set_focus()
   // {
   //
   ...
   //
   //
   // bool rdp_host::on_window_activate(int iActivate, bool bMinimized,
   ...
   //
   //
   // bool rdp_host::on_window_mouse_activate(int &iResult, const operating_system::window &operatingsystemwindowTop, int iHitTest, int iMessage)
   // {
   //
   //    iResult = MA_ACTIVATE;
   //
  ...
  //    m_pinternal->m_prdpclient->put_FullScreen(VARIANT_TRUE);
      ...
      m_ptoolbar->m_pcomwindowHost =this;

      m_ptoolbar->create_window();

      m_ptoolbar->Move(screenWidth /2 - sizeToolbar.cx / 2, 0, sizeToolbar.cx, sizeToolbar.cy);
...
//
//
//    HRESULT STDMETHODCALLTYPE rdp_host_internal::Invoke(DISPID dispIdMember, REFIID, LCID, WORD, DISPPARAMS *pDispParams,
...
//       }
      ...
      // }
      //
      // void rdp_host::OnLeaveFullScreen()
      // {
      // auto hwnd = ::as_HWND(this->operating_system_window());
      //    ShowWindow(hwnd, SW_SHOW);
      // }
      //
      // void rdp_host::OnRequestGoFullScreen()
      // {
      // auto hwnd = ::as_HWND(this->operating_system_window());
      //    ShowWindow(hwnd, SW_HIDE);
      // }
      //
      // void rdp_host::OnRequestLeaveFullScreen()
      // {
      // auto hwnd = ::as_HWND(this->operating_system_window());
...
//
//
//    void rdp_host::on_rdp_connected()
//    {
//
//  //     do_control_layout();
//
...
//
//
//    void rdp_host::on_rdp_login_complete()
//    {
//
//       //do_control_layout();
...
//
   ...
   void rdp_host::doRestore()
   {
      ...
      if (r.width() > sMainScreen.width() || r.height() > sMainScreen.height())
      {
         ...
         set_window_position(::as_operating_system_window((HWND) HWND_TOP), r.origin(), r.size(), SWP_SHOWWINDOW);
   ...
   void rdp_host::doFullscreen()
   {
      ...
      set_window_position(::as_operating_system_window((HWND) HWND_TOP), rFullscreen.origin(), rFullscreen.size(), SWP_SHOWWINDOW);
