...
{
  ::int_rectangle paintRect(paintStruct->rcPaint);
...
void DesktopWindow::doDraw(DeviceContext *dc)
{
  ...
  if (iWidth || iHeight) {
    drawBackground(dc, ::windows::as_RECT(m_clientArea), ::windows::as_RECT(::windows::as_RECT(dst)));
  }
...
void DesktopWindow::drawImage(const RECT  & rectangleSource, const RECT & rectangleTarget)
{
  ...
  if ((rectangleSource.right - rectangleSource.left) == (rectangleTarget.right - rectangleTarget.left) &&
     ...
     rectangleSource.right == rectangleTarget.right &&
     rectangleSource.top == rectangleTarget.top &&
     rectangleSource.bottom == rectangleTarget.bottom) {
...
void DesktopWindow::updateFramebuffer(const FrameBuffer * pframebuffer,
...
void DesktopWindow::setNewFramebuffer(const FrameBuffer * pframebuffer)
{
  ...
  bool isBackgroundDirty = dimension.cx < olddimension.cx ||
                           dimension.cy < olddimension.cy;
