...
namespace remoting
{
                                      ...
                                      const EncodeOptions *options)
   {
      ...
      if (bpp == 8) {
         hextileFunction<unsigned char>(rect, pframebuffer);
      } else if (bpp == 16) {
         hextileFunction<unsigned short>(rect, pframebuffer);
      } else if (bpp == 32) {
         hextileFunction<unsigned int>(rect, pframebuffer);
      } else {
         _ASSERT(0);
      }
   }
                                        ...
                                        const ::innate_subsystem::FrameBuffer *frameBuffer)
   {
      ...
      for (t.top = r.top; t.top < r.bottom; t.top += 16) {
         ...
         for (t.left = r.left; t.left < r.right; t.left += 16) {
            ...
            pframebuffer.setProperties(t, frameBuffer->getPixelFormat());
            pframebuffer.copyFrom(frameBuffer, t.left, t.top);
