...
namespace remoting_rfb_client
{
    ...
    ViewerCmdLine::ViewerCmdLine(ConnectionData *pconnectiondata,
                                 ...
                                 ::remoting_rfb::ViewerConfig *config,
                                 bool *isListening)
    ...
    void ViewerCmdLine::parseOptionsFile()
    {
        ...
        ::remoting_rfb::IniFileSettingsManager sm(pathToIniFile);
    ...
    void ViewerCmdLine::parseEncoding()
    {
        if (isPresent(ENCODING)) {
            ::i32 iEncoding = ::remoting_rfb::EncodingDefs::RAW;

            if (m_options[ENCODING] == HEXTILE) {
                iEncoding = ::remoting_rfb::EncodingDefs::HEXTILE;
            }
            if (m_options[ENCODING] == TIGHT) {
                iEncoding = ::remoting_rfb::EncodingDefs::TIGHT;
            }
            if (m_options[ENCODING] == RRE) {
                iEncoding = ::remoting_rfb::EncodingDefs::RRE;
            }
            if (m_options[ENCODING] == ZRLE) {
                iEncoding =::remoting_rfb:: EncodingDefs::ZRLE;
            }
