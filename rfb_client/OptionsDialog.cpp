...
#include "remoting/remoting_rfb/client_config/ConnectionConfig.h"
...
namespace remoting_rfb_client
{
    ...
    void OptionsDialog::setConnectionConfig(::remoting_rfb::ConnectionConfig *conConfig)
    {
    ...
    bool OptionsDialog::onInitDialog()
    {
        ...
        m_comboboxUseEnc.addItem("Raw", reinterpret_cast<void *>(::remoting_rfb::EncodingDefs::RAW));
        m_comboboxUseEnc.addItem("Hextile", reinterpret_cast<void *>(::remoting_rfb::EncodingDefs::HEXTILE));
        m_comboboxUseEnc.addItem("Tight", reinterpret_cast<void *>(::remoting_rfb::EncodingDefs::TIGHT));
        m_comboboxUseEnc.addItem("RRE", reinterpret_cast<void *>(::remoting_rfb::EncodingDefs::RRE));
        m_comboboxUseEnc.addItem("ZRLE", reinterpret_cast<void *>(::remoting_rfb::EncodingDefs::ZRLE));
    ...
    void OptionsDialog::updateControlValues()
    {
        ...
        for (::i32 i = 0; i < m_comboboxUseEnc.getItemsCount(); i++) {
            ::i32 enc = (::i32)(::iptr)m_comboboxUseEnc.getItemData(i);
            ...
            if (enc == ::remoting_rfb::EncodingDefs::HEXTILE)
                m_comboboxUseEnc.setSelectedItem(i);
        } // for i
    ...
    void OptionsDialog::onPreferredEncodingSelectionChange()
    {
        ...
        ::i32 encoding = (::i32)(::iptr)m_comboboxUseEnc.getItemData(index);
        switch (encoding) {
    ...
    void OptionsDialog::apply()
    {
        ...
        if (pesii >= 0) {
            ::i32 preferredEncoding = (::i32)(::iptr)m_comboboxUseEnc.getItemData(pesii);
            m_pconnectionconfig->setPreferredEncoding(preferredEncoding);
        } else {
            ASSERT(pesii >= 0);
            m_pconnectionconfig->setPreferredEncoding(::remoting_rfb::EncodingDefs::TIGHT);
        }
        ...
        ::i32 localCursorShape = ::remoting_rfb::ConnectionConfig::DOT_CURSOR;
        if (m_checkboxSmalldot.isChecked()) {
            localCursorShape = ::remoting_rfb::ConnectionConfig::SMALL_CURSOR;
        } else if (m_checkboxArrow.isChecked()) {
            localCursorShape = ::remoting_rfb::ConnectionConfig::NORMAL_CURSOR;
        } else if (m_checkboxNlocal.isChecked()) {
            localCursorShape = ::remoting_rfb::ConnectionConfig::NO_CURSOR;
        }
