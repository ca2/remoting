...
::i32 application::remoting_impact_main( const ::file::path & path)
{
   ...
   ::remoting_rfb::ViewerSettingsManager::initInstance(RegistryPaths::VIEWER_PATH);
   ::remoting_rfb::SettingsManager *sm = ::remoting_rfb::ViewerSettingsManager::getInstance();

   ::remoting_rfb::ViewerConfig config(RegistryPaths::VIEWER_PATH);
   config.loadFromStorage(sm);
   //auto hinstance = remoting_impact_hinstance();
   ::remoting_rfb::ConnectionConfig conConf;
