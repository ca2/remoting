...
void FileTransferReplyBuffer::onFileListReply(DataInputStream *input)
{
  ...
  if (!buffer.empty()) {
    ...
    for (unsigned int i = 0; i < m_filesInfoCount; i++) {
...
{
  unsigned char coLevel = input->readUInt8();
  unsigned int coBufferSize = input->readUInt32();
  unsigned int uncoBufferSize = input->readUInt32();
