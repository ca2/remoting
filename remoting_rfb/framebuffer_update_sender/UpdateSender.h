...
#include "remoting/remoting_rfb/rfb_sconn/Encoder.h"
#include "remoting/remoting_rfb/rfb_sconn/HextileEncoder.h"
#include "remoting/remoting_rfb/rfb_sconn/JpegEncoder.h"
#include "remoting/remoting_rfb/rfb_sconn/EncoderStore.h"
...
namespace remoting_rfb
{
   ...
   class CLASS_DECL_REMOTING_RFB  UpdateSender : public ::subsystem::Thread, public RfbDispatcherListener
   {
   public:
      ...
      ::remoting_rfb::Region m_regionRequestedIncremental;
      ::remoting_rfb::Region m_regionRequestedFull;
      bool m_incrUpdIsReq;
      bool m_fullUpdIsReq;
      bool m_busy;
      ...
      bool m_shareOnlyApp;
      ::remoting_rfb::Region m_regionApp;
      ::remoting_rfb::Region m_regionAppOld;
      ...
      // EncodeOptions class CLASS_DECL_REMOTING_RFB  maintain the configuration of encoders and
      ...
      ::remoting_rfb::Region m_regionOldVideoRegion;
      lockable_critical_section m_criticalsectionVidFreezeLoc;


      ::remoting_rfb::Region m_regionLosslessDirty;
      ::remoting_rfb::Region m_regionLosslessClean;

      // Output stream.
      ::pointer < ::remoting_rfb::RfbOutputGate > m_prfboutputgate;
      ...
      //              SenderControlInformationInterface *senderControlInformation, ::remoting_rfb::RfbOutputGate *output,
      ...
      // notifies to thread.
      // FIXME: The comment does not seem to be relevant.
      void newUpdates(const UpdateContainer & updatecontainer, const ::remoting_rfb::CursorShape *cursorShape);
   ...
   //protected:
      ...
      virtual void onRequest(::u32 reqCode, ::remoting_rfb::RfbInputGate *input) override;

      // Handlers for individual RFB client messages. Called by onRequest().
      void readUpdateRequest(::remoting_rfb::RfbInputGate *io);
      void readSetPixelFormat(::remoting_rfb::RfbInputGate *io);
      void readSetEncodings(::remoting_rfb::RfbInputGate *io);
      void readVideoFreeze(::remoting_rfb::RfbInputGate *io);
      ...
      bool extractReqRegions(::remoting_rfb::Region & incrReqReg, ::remoting_rfb::Region & fullReqReg, bool *bIncrementalUpdateRequest,
                             bool *bFullUpdateRequest, class ::time *timeReqPoint);
      UpdateContainer extractUpdates();
      void cropUpdContForReqRegions(UpdateContainer & updatecontainer, const ::remoting_rfb::Region & incrReqReg,
                                    const ::remoting_rfb::Region & fullReqReg);
      void inscribeCopiedRegionToReqRegion(UpdateContainer & updatecontainer, const ::remoting_rfb::Region & requestRegion);

      void selectEncoder(EncodeOptions & encodeoptions);

      // Updates pixels in the internal frame buffer.
      void updateFramebuffer(UpdateContainer & updatecontainer, bool shareOnlyApp, const ::remoting_rfb::Region & prevSharedRegion,
                             const ::remoting_rfb::Region & regionShareApp);
      // Updates internal view port rectangle.
      // Returns true if view port has been changed during the operation.
      bool updateViewPort(::i32_rectangle &rectangleOutNewViewport, bool *shareApp, ::remoting_rfb::Region & regionOldShareApp,
                          ::remoting_rfb::Region & regionNewShareApp);
      ...
      void paintBlack(::innate_subsystem::Framebuffer *pframebuffer, const ::remoting_rfb::Region & regionBlack);
      ...
      // encoder as well.
      void splitRegion(Encoder *encoder, const ::remoting_rfb::Region & region, ::int_rectangle_array_base & rectanglea,
                       const ::innate_subsystem::Framebuffer *pframebuffer, const EncodeOptions *encodeOptions);

      // Returns part of region with total area not much more than area
      // and removes this part form source reg
      ::remoting_rfb::Region takePartFromRegion(::remoting_rfb::Region & region, int area);
