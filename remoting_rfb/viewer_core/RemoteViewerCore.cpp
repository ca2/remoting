...
#include "remoting/remoting_rfb/file_transfer_common/FTMessage.h"
#include "remoting/remoting_rfb/rfb/AuthDefs.h"
#include "remoting/remoting_rfb/rfb/TunnelDefs.h"
#include "remoting/remoting_rfb/rfb/MsgDefs.h"
#include "remoting/remoting_rfb/rfb/EncodingDefs.h"
#include "remoting/remoting_rfb/rfb/VendorDefs.h"
...
defer_construct_newø(m_pframebuffer);
   ...
   RemoteViewerCore::RemoteViewerCore(::remoting_rfb::RfbInputGate *input, ::remoting_rfb::RfbOutputGate *output,
                                      CoreEventsAdapter *adapter,
                                      ::subsystem::LogWriter * plogwriter,
                                      bool sharedFlag)
   : m_plogwriter (plogwriter),
   ...
   void RemoteViewerCore::init()
   {
      ...
      addClientMsgCapability(::remoting_rfb::ClientMsgDefs::CLIENT_CUT_TEXT_UTF8,
        ::remoting_rfb::VendorDefs::REMOTING,
        ::remoting_rfb::Utf8CutTextDefs::CLIENT_CUT_TEXT_UTF8_SIG,
        "UTF-8 clipboard");

      addClientMsgCapability(::remoting_rfb::ClientMsgDefs::ENABLE_CUT_TEXT_UTF8,
        ::remoting_rfb::VendorDefs::REMOTING,
        ::remoting_rfb::Utf8CutTextDefs::ENABLE_CUT_TEXT_UTF8_SIG,
   ...
   void RemoteViewerCore::start(::remoting_rfb::RfbInputGate *input, ::remoting_rfb::RfbOutputGate *output,
                                CoreEventsAdapter *adapter,
                                bool sharedFlag)
   {
...
//      m_pupdaterequestsenderProperty->terminateThread();
   ...
   {
      m_isUtf8ClipboardEnabled = m_clientMsgCaps.isEnabled(::remoting_rfb::ClientMsgDefs::CLIENT_CUT_TEXT_UTF8);
      if (m_isUtf8ClipboardEnabled) {
      ...
      if (m_clientMsgCaps.isEnabled(::remoting_rfb::ClientMsgDefs::ENABLE_CUT_TEXT_UTF8)) {
         m_plogwriter->debug("Sending EnableCutTextUtf8 scopedstrMessage.");
         critical_section_lock al(m_output);
         m_output->writeUInt32(::remoting_rfb::ClientMsgDefs::ENABLE_CUT_TEXT_UTF8);
         m_output->flush();
      }
   }
   ...
   void RemoteViewerCore::enableCursorShapeUpdates(bool enabled)
   {
      ...
      if (enabled) {
         ...
         needUpdate |= m_decoderStore.removeDecoder(::remoting_rfb::PseudoEncDefs::RICH_CURSOR);
         needUpdate |= m_decoderStore.removeDecoder(::remoting_rfb::PseudoEncDefs::POINTER_POS);
      }
   ...
   void RemoteViewerCore::authenticate()
   {
      ...
      if (authenticationType != 0) {
         ...
         if (m_authHandlers.find(authenticationType) != m_authHandlers.end()) {
            ...
            if (authenticationType != ::remoting_rfb::SecurityDefs::NONE) {
      ...
      if (m_minor >= 8 || authenticationType != ::remoting_rfb::SecurityDefs::NONE) {
         ::u32 authResult = 0;
   ...
   int RemoteViewerCore::negotiateSecurityType()
   {
      ...
      if (typeSelected == ::remoting_rfb::SecurityDefs::TIGHT) {
   ...
   {
      switch (securityType) {
         case ::remoting_rfb::SecurityDefs::NONE:
            return "None";
         case ::remoting_rfb::SecurityDefs::VNC:
            return "VNC";
         case ::remoting_rfb::SecurityDefs::TIGHT:
            return "Tight";
      }
      return "Unknown type";
   }
   ...
   ::string RemoteViewerCore::getAuthenticationTypeName(::u32 authenticationType) const
   {
      switch (authenticationType) {
         case ::remoting_rfb::AuthDefs::NONE:
            return "None";
         case ::remoting_rfb::AuthDefs::VNC:
            return "VNC";
      }
      return "Unknown type";
   }
                                  ...
                                  const bool isTightEnabled) const
   {
      ...
      if (isTightEnabled) {
         ...
         SecTypesIterator tightSecType = std::find(secTypes->begin(),
                                                   secTypes->end(),
                                                   ::remoting_rfb::SecurityDefs::TIGHT);
      ...
      for (SecTypesIterator i = secTypes->begin();
         ...
         if (authHandlers->find(*i) != authHandlers->end() ||
             *i == ::remoting_rfb::SecurityDefs::NONE)
            return *i;
           }
   ...
   void RemoteViewerCore::initTunnelling()
   {
      ...
      if (tunnelCount > 0) {
         ...
         for (::u32 i = 0; i < tunnelCount; i++) {
            ...
            if (cap.code == ::remoting_rfb::TunnelDefs::NOTUNNEL) {
               hasNoTunnel = true;
            }
         ...
         if (hasNoTunnel) {
            m_output->writeUInt32(::remoting_rfb::TunnelDefs::NOTUNNEL);
            m_output->flush();
         } else {
   ...
   int RemoteViewerCore::initAuthentication()
   {
      ...
      if (authTypesNumber == 0) {
         return ::remoting_rfb::AuthDefs::NONE;
      }
   ...
   {
      try {
         // connect to host and create ::remoting_rfb::RfbInputGate/::remoting_rfb::RfbOutputGate
...
//         }
   ...
   bool RemoteViewerCore::receiveFbUpdateRectangle()
   {
      ...
      if (encodingType == ::remoting_rfb::PseudoEncDefs::LAST_RECT)
         return true;
