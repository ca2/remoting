...
namespace remoting_rfb
{
   ...
   void DesktopServerProto::initialize_desktop_server_proto(::remoting_rfb_node::Configurator * pconfigurator, BlockingGate *pblockinggate)
   {
   ...
   void DesktopServerProto::checkPixelFormat(const ::innate_subsystem::PixelFormat &pixelformat)
   {
      ...
      {
         errMess.format("Wrong value of bits per pixel ({})", (::i32)pixelformat.bitsPerPixel);
         throw ::subsystem::Exception(errMess);
      }
      if (pixelformat.colorDepth > pixelformat.bitsPerPixel)
      {
         errMess.format("Wrong value (color depth ({}) > bits per pixel ({}))", (::i32)pixelformat.colorDepth,
