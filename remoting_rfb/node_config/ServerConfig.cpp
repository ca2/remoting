...
namespace remoting_rfb_node
{
   ...
   : m_rfbPort(8900), m_httpPort(5800),
   ...
   void ServerConfig::serialize(DataOutputStream * pdataoutputstream)
   {
      ...
      ASSERT((::u32)m_videoClassNames.size() == m_videoClassNames.size());
      ...
      ASSERT((::u32)m_videoRects.size() == m_videoRects.size());
      pdataoutputstream->writeUInt32((::u32)m_videoRects.size());
      for (size_t i = 0; i < m_videoRects.size(); i++) {
         ::string s;
         ::remoting_rfb::RectSerializer::toString(m_videoRects[i], s);
   ...
   void ServerConfig::deserialize(DataInputStream * pdatainputstream)
   {
      ...
      for (size_t i = 0; i < count; i++) {
         ...
         m_videoRects.add(::remoting_rfb::RectSerializer::toRect(strVideoRect));
      }
