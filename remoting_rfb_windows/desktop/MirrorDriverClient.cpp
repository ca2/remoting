...
// FIXME: Why the class CLASS_DECL_REMOTING_RFB_WINDOWS should depence from the remoting_rfb_node_desktop project?
...
namespace remoting_rfb_windows
{
   ...
   void MirrorDriverClient::commitDisplayChanges(DEVMODE *pdm)
   {
      ...
      if (pdm)
      {
         ...
         {
            ::string errMess;
            errMess.format("1st ChangeDisplaySettingsEx() failed with code {}", (::i32)code);
            throw ::subsystem::Exception(errMess);
         }
         ...
         {
            ::string errMess;
            errMess.format("2nd ChangeDisplaySettingsEx() failed with code {}", (::i32)code);
            throw ::subsystem::Exception(errMess);
         }
      }
      else
      {
         ...
         {
            ::string errMess;
            errMess.format("ChangeDisplaySettingsEx() failed with code {}", (::i32)code);
            throw ::subsystem::Exception(errMess);
         }
      }
