  ...
  m_plogwriter(log)
{
  ...
  ::subsystem::PixelFormat termPF;
  ::int_size termDim;
  getScreenProperties(&termPF, &termDim);

  m_backupFrameBuffer.setProperties(&termDim, &termPF);

  // Synchronize our ::subsystem::FrameBuffer and the server ::subsystem::FrameBuffer
  sendInit(m_forwGate);
}
...
void UpdateHandlerClient::extract(UpdateContainer *updateContainer)
{
  ...
  try {
    ...
    if (updCont.screenSizeChanged) {
      ...
      ::subsystem::PixelFormat oldPf = m_backupFrameBuffer.getPixelFormat();
      ::int_size oldDim = m_backupFrameBuffer.getDimension();
      // Get new screen properties
      ::subsystem::PixelFormat newPf;
    ...
    if (updCont.cursorShapeChanged) {
      ...
      ::subsystem::PixelFormat newPf = m_backupFrameBuffer.getPixelFormat();
      ::int_size newDim = readDimension(m_forwGate);
...
void UpdateHandlerClient::getScreenProperties(::subsystem::PixelFormat *pf, ::int_size *dim)
{
  critical_section_lock al(m_forwGate);
