                                 ...
                                 const EncodeOptions *options)
{
  ...
  const ::subsystem_apex::FrameBuffer *clientFb = m_pixelConverter->convert(rect, serverFb);

  // Now call an encoder function corresponding to the client's pixel size.
  size_t bpp = clientFb->getBitsPerPixel();
  switch (bpp) {
  case 8:
    sendAnyRect<unsigned char>(rect, serverFb, clientFb, options);
    break;
  case 16:
    sendAnyRect<unsigned short>(rect, serverFb, clientFb, options);
    break;
  case 32:
    sendAnyRect<unsigned int>(rect, serverFb, clientFb, options);
    break;
  default:
    _ASSERT(0);
  }
                               ...
                               const EncodeOptions *options)
{
  ...
  if (numColors == 1) {
    ...
    sendIndexedRect<PIXEL_T>(rect, clientFb, options);
             ...
             rect.width() >= JPEG_MIN_RECT_WIDTH &&
             rect.height() >= JPEG_MIN_RECT_HEIGHT) {
...
void TightEncoder::sendSolidRect(const ::int_rectangle &  r, const ::subsystem_apex::FrameBuffer *fb)
{
                                   ...
                                   const EncodeOptions *options)
{
  ...
  // Prepare output buffer.
  int dataLen = rect.width() * rect.height();
  // FIXME: Optimize, use char[] instead of ByteArrayOutputStream.
  ByteArrayOutputStream encoded(dataLen);
  DataOutputStream encodedData(&encoded);
                                ...
                                const EncodeOptions *options)
{
  ...
  ::subsystem_apex::PixelFormat fmt = serverFb->getPixelFormat();
...
bool TightEncoder::shouldPackPixels(const ::subsystem_apex::PixelFormat & pf) const
{
...
void TightEncoder::packPixels(unsigned char *buf, int count, const ::subsystem_apex::PixelFormat & pf)
{
  ...
  while (count--) {
    ...
    buf += 4;
    *dst++ = (unsigned char)(pix >> pf.redShift);
    *dst++ = (unsigned char)(pix >> pf.greenShift);
    *dst++ = (unsigned char)(pix >> pf.blueShift);
  }
}

template <class PIXEL_T>
void TightEncoder::fillPalette(const ::int_rectangle &  r, const ::subsystem_apex::FrameBuffer *fb, int maxColors)
{
  // Clear the palette.
  m_pal.reset();
...
template <class PIXEL_T>
void TightEncoder::copyPixels(const ::int_rectangle &  rect, const ::subsystem_apex::FrameBuffer *fb,
                              unsigned char *dst)
{
...
template <class PIXEL_T>
void TightEncoder::encodeMonoRect(const ::int_rectangle &  rect, const ::subsystem_apex::FrameBuffer *fb,
...
template <class PIXEL_T>
void TightEncoder::encodeIndexedRect(const ::int_rectangle &  rect, const ::subsystem_apex::FrameBuffer *fb,
                                     DataOutputStream *out)
{
  const PIXEL_T *src = (const PIXEL_T *)fb->getBufferPtr(rect.left, rect.top);
  const int w = rect.width();
