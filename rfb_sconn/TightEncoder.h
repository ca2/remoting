...
protected:
  ...
  void sendSolidRect(const ::int_rectangle &  r, const FrameBuffer *fb);
  ...
  static void packPixels(unsigned char *buf, int count, const PixelFormat & pf);
  ...
  template <class PIXEL_T>
    void fillPalette(const ::int_rectangle &  r, const FrameBuffer *fb, int maxColors);

  // Copy pixel data from the frame buffer to a byte array.
  template <class PIXEL_T>
    void copyPixels(const ::int_rectangle &  rect, const FrameBuffer *fb, unsigned char *dst);
