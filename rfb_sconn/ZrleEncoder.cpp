                                ...
                                const EncodeOptions *options)
{
  ...
  //server pixel format
  ::subsystem_apex::PixelFormat serverPxFormat = serverFb->getPixelFormat();
  ...
  if (bpp == 8) {
    sendRect<unsigned char>(rect, serverFb, clientFb, options);
  } else if (bpp == 16) {
    sendRect<unsigned short>(rect, serverFb, clientFb, options);
  } else if (bpp == 32) {
    sendRect<unsigned int>(rect, serverFb, clientFb, options);
  } else {
    _ASSERT(0);
  }
}
