...
namespace remoting
{
   ...
   //     m_backupFramebuffer(backupFramebuffer), m_pcriticalsectionFramebuffer(framebufferCriticalSection), m_plogwriter = plogwriter;
   // {
   ...
   void Poller::execute()
   {
      ...
      while (!isTerminating())
      {
...

         {
            ...
            pframebufferScreen = m_pscreengrabber->getScreenBuffer();
            if (!pframebufferScreen->isEqualTo(m_backupFramebuffer))
            {
            ...
            else
            {
               ...
               for (int iRow = 0; iRow < screenHeight; iRow += pollingHeight)
               {
                  for (int iCol = 0; iCol < screenWidth; iCol += pollingWidth)
                  {
                     ...
                     if (!pframebufferScreen->cmpFrom(scanRect, m_backupFramebuffer, scanRect.left, scanRect.top))
                     {
