...
namespace remoting
{
   ...
   MirrorScreenDriver::MirrorScreenDriver(UpdateKeeper * pupdatekeeper, UpdateListener * pupdatelistener,
       ...
       UpdateDetector(pupdatekeeper, pupdatelistener), m_pcriticalsectionFramebuffer(fbcritical_section), m_lastCounter(0), m_plogwriter(plogwriter)
   {
   ...
   bool MirrorScreenDriver::grab(const ::int_rectangle & rectangle)
   {
      ...
      if (rectangle != 0)
      {
         croppedRect = rectangle.intersection(&fbRect);
      }
      else
      {
         croppedRect = fbRect;
   ...
   bool MirrorScreenDriver::applyNewProperties()
   {
      ...
      ::innate_subsystem::PixelFormat pixelformat = m_mirrorClient->getPixelFormat();
      m_pframebuffer.setProperties(&newDim, &pixelformat);
      m_lastCounter = 0;
