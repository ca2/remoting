...
bool CommonInputValidation::validatePort(TextBox *textBox)
{
  ...
  if (!validationOk) {
    notifyValidationError(textBox, MainSubsystem().string_table()->getString(IDS_PORT_RANGE_ERROR));
  }
...
void CommonInputValidation::notifyValidationError(TextBox *textBox, const ::scoped_string & scopedstrMessageText)
{
  ...
  invalidInputTip.setTitle(MainSubsystem().string_table()->getString(IDS_CAPTION_BAD_INPUT));
