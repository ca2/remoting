...
#include "remoting/remoting_rfb/node/ControlCommand.h"
...
#include "remoting/rfb_node_desktop/ServerHelp.h"
...
#include "remoting/remoting_rfb/node_user_config/ConfigDialog.h"
...
m_commandLine(scopedstrCommandLine)
   ...
   void ControlApplication::onThreadMain()
   {
      ...
      if (cmdLineParser.hasControlServiceFlag() && cmdLineParser.isSlave()) {
         try {
            appGlobalMutex = node()->create_local_named_mutex(this, false, ::remoting_rfb_node::ServerApplicationNames::CONTROL_APP_INSTANCE_MUTEX_NAME);
         } catch (...) {
            setExitCode(1);
            return;
         }
      ...
      if (cmdLineParser.isCommandSpecified()) {
         ...
         if (cmdLineParser.isSlave()) {
            ...
            try {
               ...
               } catch (::remoting_rfb_node::RemoteException &remEx) {
...
return;
   ...
   {
      ::remoting_rfb_node::ControlCommand ctrlCmd(command);
   ...
   int ControlApplication::runConfigurator(bool configService, bool isRunAsRequested)
   {
      ...
      if (configService && (MainSubsystem().OperatingSystem().isUserAnAdmin() == false)) {
         ...
         // Path to remoting_rfb_node binary.
         ::string pathToBinary;
      ...
      ::remoting_rfb_node::ConfigDialog confDialog(configurator, nullptr);
