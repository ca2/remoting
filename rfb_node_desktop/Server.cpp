...
///#include "remoting/remoting_rfb/network/socket/WindowsSocket.h"
...
#include "remoting/rfb_node_desktop/control_desktop/TransportFactory.h"
#include "remoting/rfb_node_desktop/control_desktop/ControlPipeName.h"

#include "remoting/rfb_node_desktop/BuildTime.h"
...
namespace remoting_rfb_node_desktop
{
   ...
   void Server::initialize_remoting_node_desktop_server(bool runsInServiceContext, ::remoting_rfb_node::NewConnectionEvents *newConnectionEvents, LogInitListener *logInitListener,
                  ::subsystem::LogWriter *plogwriter)
   {
      //: Singleton<Server>(),
       //:
      ...
      m_plogwriter->information("{} Build on {}", ::remoting_rfb_node::ProductNames::SERVER_PRODUCT_NAME, BuildTime::DATE);

      // Initialize configuration.
      // FIXME: It looks like configurator may be created as a member object.
      ::remoting_rfb_node::Configurator *configurator = m_pconfigurator;
      configurator->load();
      m_pserverconfig = m_pconfigurator->getServerConfig();
      ...
      try
      {
         ...
         ::u8 logLevel = m_pserverconfig->getLogLevel();
         // FIXME: Use correct log name.
         m_ploginitlistener->onLogInit(logDir, ::remoting_rfb_node::LogNames::SERVER_LOG_FILE_STUB_NAME, logLevel);
      }
      catch (...)
      {
         // A log error must not be a reason that stop the server.
   ...
   void Server::onConfigReload(::remoting_rfb_node::ServerConfig *serverConfig)
   {
   ...
   void Server::afterLastClientDisconnect()
   {
...
#endif
      ::remoting_rfb_node::ServerConfig::DisconnectAction action = m_pserverconfig->getDisconnectAction();
      ...
      switch (action) {
         case ::remoting_rfb_node::ServerConfig::DA_LOCK_WORKSTATION:
            keys.format("{}", AdditionalActionApplication::LOCK_WORKSTATION_KEY);
            break;
         case ::remoting_rfb_node::ServerConfig::DA_LOGOUT_WORKSTATION:
