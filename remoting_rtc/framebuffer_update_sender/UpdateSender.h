...
namespace remoting
{
   ...
   //protected:
      ...
      // encoder as well.
      void splitRegion(Encoder *encoder, const ::remoting_rtc::Region & region, ::int_rectangle_array_base & rectanglea,
                       const ::innate_subsystem::Framebuffer *pframebuffer, const EncodeOptions *encodeOptions);

      // Returns part of region with total area not much more than area
      // and removes this part form source reg
      ::remoting_rtc::Region takePartFromRegion(::remoting_rtc::Region & region, int area);
