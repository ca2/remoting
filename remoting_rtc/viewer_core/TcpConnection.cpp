...
m_RfbGatesOwner(false)
   ...
   void TcpConnection::bind(::remoting_rtc::RfbInputGate * pinput, ::remoting_rtc::RfbOutputGate * poutput)
   {
      critical_section_lock al(&m_connectLock);
      if (m_wasBound) {
         throw ::subsystem::Exception("Tcp-connection already bound");
   ...
   ::remoting_rtc::RfbInputGate *TcpConnection::getInput() const
   {
      {
         critical_section_lock al(&m_connectLock);
         if (!m_isEstablished) {
   ...
   ::remoting_rtc::RfbOutputGate *TcpConnection::getOutput() const
   {
      {
         critical_section_lock al(&m_connectLock);
         if (!m_isEstablished) {
