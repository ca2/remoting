...
#include "remoting/remoting_rtc/rfb/AuthDefs.h"
#include "remoting/remoting_rtc/rfb/TunnelDefs.h"
#include "remoting/remoting_rtc/rfb/MsgDefs.h"
...
defer_construct_newø(m_pframebuffer);
   ...
   RemoteViewerCore::RemoteViewerCore(::remoting_rtc::RfbInputGate *input, ::remoting_rtc::RfbOutputGate *output,
                                      CoreEventsAdapter *adapter,
                                      ::subsystem::LogWriter * plogwriter,
                                      bool sharedFlag)
   : m_plogwriter (plogwriter),
   ...
   void RemoteViewerCore::init()
   {
      ...
      addClientMsgCapability(::remoting_rtc::ClientMsgDefs::CLIENT_CUT_TEXT_UTF8,
        ::remoting_rtc::VendorDefs::REMOTING,
        ::remoting_rtc::Utf8CutTextDefs::CLIENT_CUT_TEXT_UTF8_SIG,
        "UTF-8 clipboard");

      addClientMsgCapability(::remoting_rtc::ClientMsgDefs::ENABLE_CUT_TEXT_UTF8,
        ::remoting_rtc::VendorDefs::REMOTING,
        ::remoting_rtc::Utf8CutTextDefs::ENABLE_CUT_TEXT_UTF8_SIG,
   ...
   void RemoteViewerCore::start(::remoting_rtc::RfbInputGate *input, ::remoting_rtc::RfbOutputGate *output,
                                CoreEventsAdapter *adapter,
                                bool sharedFlag)
   {
...
//      m_pupdaterequestsenderProperty->terminateThread();
   ...
   {
      m_isUtf8ClipboardEnabled = m_clientMsgCaps.isEnabled(::remoting_rtc::ClientMsgDefs::CLIENT_CUT_TEXT_UTF8);
      if (m_isUtf8ClipboardEnabled) {
      ...
      if (m_clientMsgCaps.isEnabled(::remoting_rtc::ClientMsgDefs::ENABLE_CUT_TEXT_UTF8)) {
         m_plogwriter->debug("Sending EnableCutTextUtf8 scopedstrMessage.");
         critical_section_lock al(m_output);
         m_output->writeUInt32(::remoting_rtc::ClientMsgDefs::ENABLE_CUT_TEXT_UTF8);
         m_output->flush();
      }
   }
   ...
   void RemoteViewerCore::enableCursorShapeUpdates(bool enabled)
   {
      ...
      if (enabled) {
         ...
         needUpdate |= m_decoderStore.removeDecoder(::remoting_rtc::PseudoEncDefs::RICH_CURSOR);
         needUpdate |= m_decoderStore.removeDecoder(::remoting_rtc::PseudoEncDefs::POINTER_POS);
      }
   ...
   void RemoteViewerCore::authenticate()
   {
      ...
      if (authenticationType != 0) {
         ...
         if (m_authHandlers.find(authenticationType) != m_authHandlers.end()) {
            ...
            if (authenticationType != ::remoting_rtc::SecurityDefs::NONE) {
      ...
      if (m_minor >= 8 || authenticationType != ::remoting_rtc::SecurityDefs::NONE) {
         ::u32 authResult = 0;
   ...
   int RemoteViewerCore::negotiateSecurityType()
   {
      ...
      if (typeSelected == ::remoting_rtc::SecurityDefs::TIGHT) {
   ...
   {
      switch (securityType) {
         case ::remoting_rtc::SecurityDefs::NONE:
            return "None";
         case ::remoting_rtc::SecurityDefs::VNC:
            return "VNC";
         case ::remoting_rtc::SecurityDefs::TIGHT:
            return "Tight";
      }
      return "Unknown type";
   }
   ...
   ::string RemoteViewerCore::getAuthenticationTypeName(::u32 authenticationType) const
   {
      switch (authenticationType) {
         case ::remoting_rtc::AuthDefs::NONE:
            return "None";
         case ::remoting_rtc::AuthDefs::VNC:
            return "VNC";
      }
      return "Unknown type";
   }
                                  ...
                                  const bool isTightEnabled) const
   {
      ...
      if (isTightEnabled) {
         ...
         SecTypesIterator tightSecType = std::find(secTypes->begin(),
                                                   secTypes->end(),
                                                   ::remoting_rtc::SecurityDefs::TIGHT);
      ...
      for (SecTypesIterator i = secTypes->begin();
         ...
         if (authHandlers->find(*i) != authHandlers->end() ||
             *i == ::remoting_rtc::SecurityDefs::NONE)
            return *i;
           }
   ...
   void RemoteViewerCore::initTunnelling()
   {
      ...
      if (tunnelCount > 0) {
         ...
         for (::u32 i = 0; i < tunnelCount; i++) {
            ...
            if (cap.code == ::remoting_rtc::TunnelDefs::NOTUNNEL) {
               hasNoTunnel = true;
            }
         ...
         if (hasNoTunnel) {
            m_output->writeUInt32(::remoting_rtc::TunnelDefs::NOTUNNEL);
            m_output->flush();
         } else {
   ...
   int RemoteViewerCore::initAuthentication()
   {
      ...
      if (authTypesNumber == 0) {
         return ::remoting_rtc::AuthDefs::NONE;
      }
   ...
   {
      try {
         // connect to host and create ::remoting_rtc::RfbInputGate/::remoting_rtc::RfbOutputGate
...
//         }
   ...
   bool RemoteViewerCore::receiveFbUpdateRectangle()
   {
      ...
      if (encodingType == ::remoting_rtc::PseudoEncDefs::LAST_RECT)
         return true;
