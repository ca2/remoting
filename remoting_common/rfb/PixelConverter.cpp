...
namespace remoting
{
   ...
   void PixelConverter::convert(const ::int_rectangle &  rect, ::subsystem_apex::FrameBuffer *dstFb,
                                const ::subsystem_apex::FrameBuffer *srcFb) const
   {
      if (m_convertMode == NO_CONVERT) {
         ...
         ::subsystem_apex::PixelFormat dstPf = dstFb->getPixelFormat();
         ::subsystem_apex::PixelFormat srcPf = srcFb->getPixelFormat();

         unsigned int dstPixelSize = dstPf.bitsPerPixel / 8;
         unsigned int srcPixelSize = srcPf.bitsPerPixel / 8;

         // FIXME: Make ::subsystem_apex::FrameBuffer do the math.
   ...
   const ::subsystem_apex::FrameBuffer *
   PixelConverter::convert(const ::int_rectangle &  rect, const ::subsystem_apex::FrameBuffer *srcFb)
   {
      if (m_convertMode == NO_CONVERT) {
         return srcFb;
      }
      ...
      if (m_dstFrameBuffer == 0) {
         ...
         m_dstFrameBuffer = new ::subsystem_apex::FrameBuffer;
   ...
   void PixelConverter::setPixelFormats(const ::subsystem_apex::PixelFormat & dstPf,
   ...
   void PixelConverter::fillHexBitsTable(const ::subsystem_apex::PixelFormat & dstPf,
                                         const ::subsystem_apex::PixelFormat & srcPf)
   {
   ...
   void PixelConverter::fill32BitsTable(const ::subsystem_apex::PixelFormat & dstPf,
                                        const ::subsystem_apex::PixelFormat & srcPf)
   {
      unsigned int dstRedMax = dstPf.redMax;
      unsigned int dstGrnMax = dstPf.greenMax;
