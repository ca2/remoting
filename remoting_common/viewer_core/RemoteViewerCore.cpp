...
#include "remoting/remoting_common/rfb/TunnelDefs.h"
                                                ...
                                                int encodingType)
   {
      switch (encodingType) {
         ...
         case PseudoEncDefs::RICH_CURSOR:
         {
            ...
            ::int_point hotSpot(rect.left, rect.top);
         ...
         {
            m_plogwriter->debug("Updating pointer position: [{}, {}]", rect.left, rect.top);
            ::int_point position(rect.left, rect.top);
            m_fbUpdateNotifier.updatePointerPos(&position);
         }
            break;
