...
m_RfbGatesOwner(false)
   ...
   void TcpConnection::bind(::remoting_rfb::RfbInputGate * pinput, ::remoting_rfb::RfbOutputGate * poutput)
   {
      critical_section_lock al(&m_connectLock);
      if (m_wasBound) {
         throw ::subsystem::Exception("Tcp-connection already bound");
   ...
   ::remoting_rfb::RfbInputGate *TcpConnection::getInput() const
   {
      {
         critical_section_lock al(&m_connectLock);
         if (!m_isEstablished) {
   ...
   ::remoting_rfb::RfbOutputGate *TcpConnection::getOutput() const
   {
      {
         critical_section_lock al(&m_connectLock);
         if (!m_isEstablished) {
