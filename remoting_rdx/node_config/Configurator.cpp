...
//#include "remoting/remoting_rfb/node_user_config/TvnLogFilename.h"
...
namespace remoting_rfb_node
{
   ...
   // }
   //
   // void Configurator::setInstance(::remoting_rfb_node::Configurator *conf) { s_instance = conf; }
   ...
   bool Configurator::load(bool forService)
   {
      ...
      ::remoting_rfb::RegistrySettingsManager sm(pregistrykey, "Software/remoting/server");
   ...
   bool Configurator::save(bool forService)
   {
      ...
      //   sa = m_regSA->getServiceSA();
      // }
      //::remoting_rfb::RegistrySettingsManager sm(rootKey, RegistryPaths::SERVER_PATH, sa);
       ::remoting_rfb::RegistrySettingsManager sm(pregistrykey,"Software/remoting/server");
   ...
   bool Configurator::save(::remoting_rfb::SettingsManager *sm)
   {
   ...
   bool Configurator::load(::remoting_rfb::SettingsManager *sm)
   {
   ...
   bool Configurator::savePortMappingContainer(::remoting_rfb::SettingsManager *sm)
   {
      bool saveResult = true;
   ...
   bool Configurator::loadPortMappingContainer(::remoting_rfb::SettingsManager *sm, PortMappingContainer *portMapping)
   {
      bool wasError = false;
   ...
   bool Configurator::saveQueryConfig(::remoting_rfb::SettingsManager *sm)
   {
      bool saveResult = true;
   ...
   bool Configurator::loadQueryConfig(::remoting_rfb::SettingsManager *sm, ::remoting_rfb_node::ServerConfig * pserverconfig)
   {
      bool loadResult = true;
      ::u32 uintValue;
      bool boolValue;
   ...
   bool Configurator::saveInputHandlingConfig(::remoting_rfb::SettingsManager *sm)
   {
      bool saveResult = true;
   ...
   bool Configurator::loadInputHandlingConfig(::remoting_rfb::SettingsManager *sm, ::remoting_rfb_node::ServerConfig * pserverconfig)
   {
      bool loadResult = true;
   ...
   bool Configurator::saveVideoRegionConfig(::remoting_rfb::SettingsManager *sm)
   {
      bool saveResult = true;
      ...
      {
         ::string s;
         ::remoting_rfb::RectSerializer::toString(rectangleaVideo->at(i), s);
         buffer += s;
         if (i != size - 1)
         {
            buffer += "\n";
   ...
   bool Configurator::loadVideoRegionConfig(::remoting_rfb::SettingsManager *sm, ::remoting_rfb_node::ServerConfig * pserverconfig)
   {
      bool loadResult = true;
      ...
      if (chunks.has_element())
      {
         ...
         for (size_t i = 0; i < count; i++)
         {
            if (!chunks[i].is_empty())
            {
               rectangleaVideo->add(::remoting_rfb::RectSerializer::toRect(chunks[i]));
            }
         }
      }
      return loadResult;
   ...
   bool Configurator::saveIpAccessControlContainer(::remoting_rfb::SettingsManager *storage)
   {
      AutoLock l(m_pserverconfig.m_p);
   ...
   bool Configurator::loadIpAccessControlContainer(::remoting_rfb::SettingsManager *sm, IpAccessControl *rules)
   {
      bool wasError = false;
      rules->clear();
   ...
   bool Configurator::saveServerConfig(::remoting_rfb::SettingsManager *sm)
   {
      bool saveResult = true;
   ...
   bool Configurator::loadServerConfig(::remoting_rfb::SettingsManager *sm, ::remoting_rfb_node::ServerConfig * pserverconfig)
   {
      bool loadResult = true;
      ...
      {
         m_isConfigLoadedPartly = true;
         m_pserverconfig->setDisconnectAction((::remoting_rfb_node::ServerConfig::DisconnectAction)uintVal);
      }
