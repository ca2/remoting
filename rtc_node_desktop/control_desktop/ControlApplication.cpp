...
#include "remoting/remoting_rtc/node/ControlCommand.h"
...
#include "remoting/remoting_rtc/node_user_config/ConfigDialog.h"
...
m_commandLine(scopedstrCommandLine)
   ...
   void ControlApplication::onThreadMain()
   {
      ...
      if (cmdLineParser.hasControlServiceFlag() && cmdLineParser.isSlave()) {
         try {
            appGlobalMutex = node()->create_local_named_mutex(this, false, ::remoting_rtc_node::ServerApplicationNames::CONTROL_APP_INSTANCE_MUTEX_NAME);
         } catch (...) {
            setExitCode(1);
            return;
         }
      ...
      if (cmdLineParser.isCommandSpecified()) {
         ...
         if (cmdLineParser.isSlave()) {
            ...
            try {
               ...
               } catch (::remoting_rtc_node::RemoteException &remEx) {
...
return;
   ...
   {
      ::remoting_rtc_node::ControlCommand ctrlCmd(command);
   ...
   ::i32 ControlApplication::runConfigurator(bool configService, bool isRunAsRequested)
   {
      ...
      if (configService && (MainSubsystem().OperatingSystem().isUserAnAdmin() == false)) {
         ...
         // Path to remoting_rtc_node binary.
         ::string pathToBinary;
         // Command line for child process.
         ::string childCommandLine;

         // Get path to remoting_rtc_node binary.
      ...
      ::remoting_rtc_node::ConfigDialog confDialog(configurator, nullptr);
