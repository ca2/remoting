...
namespace remoting_node_desktop
{
       ...
       m_plogwriter(::system())
   {
      ...
      try
      {
         ...
         m_pgateClientToServer = new ::remoting_rtc::BlockingGate(m_pchannelClientToServer);
         m_pgateServerToClient = new ::remoting_rtc::BlockingGate(m_pchannelServerToClient);

         // Server initializations
         m_pdesktopsrvdispatcher = new ::remoting_rtc::DesktopSrvDispatcher(m_pgateClientToServer, [this]() { onHappening(); }, m_plogwriter);

         m_pupdatehandlerserver = allocateø ::remoting_rtc::UpdateHandlerServer();
         ...
         m_puserinputserver = allocateø ::remoting_rtc::UserInputServer(m_pconfigurator, m_pgateServerToClient, m_pdesktopsrvdispatcher, [this]() { onHappening(); }, m_plogwriter);

         m_pconfigserver = allocateø ::remoting_rtc::ConfigServer();
         m_pconfigserver->initialize_config_server(m_pconfigurator, m_pdesktopsrvdispatcher, m_plogwriter);

         m_pgatekickhandler = allocateø ::remoting_rtc::GateKickHandler();
   ...
   void DesktopServerApplication::onConfigReload(::remoting_rtc_node::ServerConfig *serverConfig) {}
   ...
   void DesktopServerApplication::onOperatingSystemApplicationMain()
   {
      try
      {
         ...
         auto pwallpaperutil = createø<::remoting_rtc::WallpaperUtil>();
