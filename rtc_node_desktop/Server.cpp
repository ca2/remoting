...
///#include "remoting/remoting_rtc/network/socket/WindowsSocket.h"
...
namespace remoting_node_desktop
{
   ...
   void Server::initialize_remoting_node_desktop_server(bool runsInServiceContext, ::remoting_rtc_node::NewConnectionEvents *newConnectionEvents, LogInitListener *logInitListener,
                  ::subsystem::LogWriter *plogwriter)
   {
      //: Singleton<Server>(),
       //:
      ...
      m_plogwriter->information("{} Build on {}", ::remoting_rtc_node::ProductNames::SERVER_PRODUCT_NAME, BuildTime::DATE);

      // Initialize configuration.
      // FIXME: It looks like configurator may be created as a member object.
      ::remoting_rtc_node::Configurator *configurator = m_pconfigurator;
      configurator->load();
      m_pserverconfig = m_pconfigurator->getServerConfig();
      ...
      try
      {
         ...
         unsigned char logLevel = m_pserverconfig->getLogLevel();
         // FIXME: Use correct log name.
         m_ploginitlistener->onLogInit(logDir, ::remoting_rtc_node::LogNames::SERVER_LOG_FILE_STUB_NAME, logLevel);
      }
      catch (...)
      {
         // A log error must not be a reason that stop the server.
   ...
   void Server::onConfigReload(::remoting_rtc_node::ServerConfig *serverConfig)
   {
   ...
   void Server::afterLastClientDisconnect()
   {
...
#endif
      ::remoting_rtc_node::ServerConfig::DisconnectAction action = m_pserverconfig->getDisconnectAction();
      ...
      switch (action) {
         case ::remoting_rtc_node::ServerConfig::DA_LOCK_WORKSTATION:
            keys.format("{}", AdditionalActionApplication::LOCK_WORKSTATION_KEY);
            break;
         case ::remoting_rtc_node::ServerConfig::DA_LOGOUT_WORKSTATION:
